<#
    .SYSNOPSIS
        Populate the local profile information into the WMI
    ...
    .NOTES
        Name: Invoke-RFLProfileInformationToWMI.ps1
        Author: Raphael Perez
        DateCreated: 18 December 2020 (v0.1)
        Update: 18 December 2020 (0.2)
            - Added ProfileSizeMB
        LastUpdate: 18 December 2020 (v0.2)

    .EXAMPLE
        .\Invoke-RFLProfileInformationToWMI.ps1
        .\Invoke-RFLProfileInformationToWMI.ps1 -Namespace 'corp' -Class 'AdminUsers'
        .\Invoke-RFLProfileInformationToWMI.ps1 -Namespace 'corp' -Class 'LocalAdmin'
#>
#requires -version 5
[CmdletBinding()]
param (
...
$script:LogFilePath = $env:Temp
$Script:LogFileFileName = 'Invoke-RFLProfileInformationToWMI.log'
$script:ScriptLogFilePath = "$($script:LogFilePath)\$($Script:LogFileFileName)"
$Script:ClassAttributes = @('ID', 'lastusetime', 'localpath', 'SID', 'distinguishedName', 'name', 'homeDirectory', 'sAMAccountName', 'userPrincipalName', 'Date', 'Version', 'profileSizeMB')
#endregion
...
try {
    ...
    Write-RFLLog -Message "Getting list of all profiles"
    $ProfileList = get-ciminstance win32_userprofile | ? lastusetime | select lastusetime, loaded, localpath, SID

    $ProfileList | ForEach-Object {
        $item = $_
        Write-RFLLog -Message "Checking username for SID $($item.SID)"
        $obj = [ADSI]("LDAP://<SID=$($item.SID)>")
        ...
        $userPrincipalName = ''
        try {
            $distinguishedName = $obj.distinguishedName.ToString()
            $name = $obj.name.ToString()
            $homeDirectory = $obj.homeDirectory.ToString()
            $sAMAccountName = $obj.sAMAccountName.ToString()
            $userPrincipalName = $obj.userPrincipalName.ToString()
        } catch {
        }

        Write-RFLLog -Message "Checking profile size"
        [UInt64]$profileSize = ( Get-Childitem -Path $item.localpath -Force -Recurse -File -ErrorAction SilentlyContinue | Measure-Object -Property "Length" -Sum ).Sum
