<#
    ...
    .EXAMPLE
        .\Remediate-BitlockerKeyToAAD.ps1
#>
#requires -version 5
[CmdletBinding()]
param(
...
$script:LogFilePath = $env:Temp
$Script:LogFileFileName = 'Remediate-BitlockerKeyToAAD.log'
$script:ScriptLogFilePath = "$($script:LogFilePath)\$($Script:LogFileFileName)"
...
try {
    ...
    Write-RFLLog -Message "Checking Bitlocker Status"

    $BLSysVolume = Get-BitLockerVolume -MountPoint $env:SystemDrive -ErrorAction Stop
    Write-RFLLog -Message "protected system volume is '$($BLSysVolume.MountPoint)' and current status is '$($BLSysVolume.VolumeStatus)'"
    ...
    Write-RFLLog -Message "Recovery Protector ID: '$($BLprotectorguid)'"

    $missingBackups = 0
    foreach ($guid in $BLprotectorguid) {
        Write-RFLLog -Message "Checking backup event for '$($BLprotectorguid)'"
        ...
        if ($BLBackupEvent) {
            Write-RFLLog -Message "backup event found. no need to remediate"
        } else {
            Write-RFLLog -Message "Missing backup event. Forcing Backup to AAD" -logLevel 3
            BackuptoAAD-BitLockerKeyProtector -MountPoint $env:SystemDrive -KeyProtectorId $BLRecoveryProtector.KeyProtectorID -ErrorAction Stop | Out-Null
            Write-RFLLog -Message "Backup event initiated."
