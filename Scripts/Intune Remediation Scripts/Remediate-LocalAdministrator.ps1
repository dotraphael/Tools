...
try {
    ...
    if ($Account) {
        ...
        Write-Verbose "User added to the local administrator group"

        Write-RFLLog -Message "Checking Account creation"
        $Account = $Account = Get-LocalUser -Name $username -ErrorAction SilentlyContinue

        if ($Account) {
            Write-RFLLog -Message "Account created successfully"
        } else {
            Write-RFLLog -Message "Account creation failed. Check computer." -LogLevel 3
        }
    }
} catch {
