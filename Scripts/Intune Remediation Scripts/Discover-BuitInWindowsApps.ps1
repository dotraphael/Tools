<#
    .SYSNOPSIS
        Discover Builtin Windows Apps, Capabilities Optional Features
    ...
    .NOTES
        Name: Discover-BuitInWindowsApps.ps1
    ...
    .EXAMPLE
        .\Discover-BuitInWindowsApps.ps1
#>
#requires -version 5
[CmdletBinding()]
param(
...
$script:LogFilePath = $env:Temp
$Script:LogFileFileName = 'Discover-BuitInWindowsApps.log'
$script:ScriptLogFilePath = "$($script:LogFilePath)\$($Script:LogFileFileName)"
...
try {
    ...
    ForEach ($item in $Script:CapabilityList) {
        ...
        if ($Capability.State -eq 'Installed') {
            Write-RFLLog -Message "capability '$($item)' present. Need to uninstall" -LogLevel 2
        	$Found = $True
        } else {
    ...
    ForEach ($item in $Script:Opplist) {
        ...
        if ($Opp.State -eq 'Enabled') {
            Write-RFLLog -Message "OptionalFeature '$($item)' present. Need to disable" -LogLevel 2
            $Found = $True
	    } else {
    ...
    if ($Found) {
        Write-RFLLog -Message 'capabilities, App or Optional Features exist. Remediation script must be run' -LogLevel 2
        Write-RFLLog -Message "*** Ending ***"
        Exit 1 
    } else {
        Write-RFLLog -Message 'capabilities, App or Optional Features do NOT exist. Remediation script do not need to be run'
        Write-RFLLog -Message "*** Ending ***"
        Write-host 'capabilities, App or Optional Features do NOT exist. Remediation script do not need to be run'
        Exit 0 
    }
}
#endregion
