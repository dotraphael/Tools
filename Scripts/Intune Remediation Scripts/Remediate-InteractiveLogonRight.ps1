<#
	...
	.DESCRIPTION
        ...
        Use Remediation on demand: https://learn.microsoft.com/en-us/mem/intune/fundamentals/remediations#run-a-remediation-script-on-demand-preview
...
PROCESS {
    ...
    if ($ComputerName -or $Username -or $UserRight -or $RemoveRight) {
	   ...
	   #>
       Set-UserRights -RemoveRight -ComputerName $env:COMPUTERNAME -UserRight SeInteractiveLogonRight -Username users
       shutdown -r -t 5
    }
}
END {
