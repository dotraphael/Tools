<#
    ...
    .PARAMETER Folder
        Folder that contains the wifi profiles (xml file)

    .NOTES
        Name: Import-RFLWifiProfile.ps1
        Author: Raphael Perez
        DateCreated: 29 April 2020 (v0.1)

    .EXAMPLE
        .\Import-RFLWifiProfile.ps1 -Folder 'c:\temp\wifi'
#>
#requires -version 5
[CmdletBinding()]
param(
...
$script:LogFilePath = $env:Temp
$Script:LogFileFileName = 'Import-RFLWifiProfile.log'
$script:ScriptLogFilePath = "$($script:LogFilePath)\$($Script:LogFileFileName)"
#endregion
...
try {
    ...
    Write-RFLLog -Message "Folder as $($Folder)"

    Get-ChildItem $Folder -Recurse -Filter *.xml | ForEach-Object {
        Write-RFLLog -Message "Adding profile $($_.FullName)"
