<#
    .SYSNOPSIS
        Add Computer object to Active Directory Group without AD module

    .DESCRIPTION
        This finds the computer object anywhere in AD and adds it to a security group in a known location

    .PARAMETER ADGroupLdap
        AD Group LDAP Information (format: CN=GroupName,OU=GROUPS,DC=DOMAIN,DC=LOCAL)
    ...
    .NOTES
        Name: Invoke-RFLAddComputerToGroup.ps1
        Author: Raphael Perez
        DateCreated: 29 April 2021 (v0.1)

    .EXAMPLE
        .\Invoke-RFLAddComputerToGroup.ps1 -ADGroupLdap 'CN=GroupName,OU=GROUPS,DC=DOMAIN,DC=LOCAL'
        .\Invoke-RFLAddComputerToGroup.ps1 -ADGroupLdap 'CN=GroupName,OU=GROUPS,DC=DOMAIN,DC=LOCAL' -ADAccountUserName 'domain\username' -ADAccountPassword 'password'
#>
#requires -version 5
[CmdletBinding()]
param(
...
$script:LogFilePath = $env:Temp
$Script:LogFileFileName = 'Invoke-RFLAddComputerToGroup.log'
$script:ScriptLogFilePath = "$($script:LogFilePath)\$($Script:LogFileFileName)"
#endregion
...
try {
    ...
    Write-RFLLog -Message "Setting Filter"
    $ds.Filter = "(&(objectClass=computer)(sAMAccountName= $($env:computername)`$)(memberof=$($ADGroupLdap)))"

    Write-RFLLog -Message "Check to see if computer is already a member of the group"
    $isMemberResult = $ds.FindOne()

    #If the computer is already a member of the group, just exit.
    If ($isMemberResult) {
        Write-RFLLog -Message "Computer is already member of the gorup. No further action taken" -LogLevel 2
    } else {
        Write-RFLLog -Message "Computer is NOT member of the gorup. Adding it"
        Write-RFLLog -Message "Setting Filter"
        ...
        if ($ADAccountUserName) {
            $Group = New-Object System.DirectoryServices.DirectoryEntry("LDAP://$($ADGroupLdap)", $ADAccountUserName, $ADAccountPassword)
        } else {
            $Group = [ADSI]"LDAP://$($ADGroupLdap)"
        }

        if ($Group) {
            Write-RFLLog -Message "Group Object Found: $($Group.Path)"
        } else {
            Write-RFLLog -Message "Group Object NOT found. No Further aciton taken" -LogLevel 3
            Exit 3000
        }

        Write-RFLLog -Message "Adding Computer object to group"
        $Group.Add($ComputerPath)
        $Group.SetInfo()

        Write-RFLLog -Message "Checking if computer has been added to the group"
        Write-RFLLog -Message "Setting Filter"
        $ds.Filter = "(&(objectClass=computer)(sAMAccountName= $($env:computername)`$)(memberof=$($ADGroupLdap)))"
        ...
        If ($isMemberResult) { 
            Write-RFLLog -Message "Computer added to the group"
        } else {
            Write-RFLLog -Message "Computer was not added to the group" -LogLevel 3
            Exit 3000
        }
    }
} catch {
