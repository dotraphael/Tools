<#
    ...
    .PARAMETER EPPolicyPath
        Full EP Policy path

    .NOTES
        Name: Set-RFLEPSettings.ps1
        Author: Raphael Perez
        DateCreated: 01 May 2020 (v0.1)

    .EXAMPLE
        .\Set-RFLEPSettings.ps1 -EPPolicyPath 'c:\temp\EP.xml'
#>
#requires -version 5
[CmdletBinding()]
param(
...
$script:LogFilePath = $env:Temp
$Script:LogFileFileName = 'Set-RFLEPSettings.log'
$script:ScriptLogFilePath = "$($script:LogFilePath)\$($Script:LogFileFileName)"
#endregion
...
try {
    ...
    Write-RFLLog -Message "Parameter EPPolicy is $($EPPolicyPath)"
    Write-RFLLog -Message "Root Folder: $(Get-ScriptDirectory)"

    Write-RFLLog -Message "Applying policy $($EPPolicyPath)"

    Set-ProcessMitigation -PolicyFilePath "$($EPPolicyPath)" -ErrorAction Stop
} catch {
